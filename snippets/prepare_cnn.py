...
def prepare(
    destination_path: Path = Path("data/cnn"),
    test_split_fraction: float = 0.1,
    checkpoint_dir: Path = Path("checkpoints/stabilityai/stablelm-base-alpha-3b"),
    mask_inputs: bool = False,  # as in alpaca-lora
    seed: int = 42,
    include_multiturn_conversations: bool = False,
    data_repo_id: str = "cnn_dailymail",
    ignore_index: int = -1,
    access_token: Optional[str] = os.getenv("HF_TOKEN"),
    max_seq_length: Optional[int] = None,
) -> None:
    """Prepare the cnn/dailymail dataset for instruction tuning.
    ...
    dataset = load_dataset(data_repo_id, "3.0.0", split="train[:2000]", use_auth_token=access_token)
    train_data = format_dataset(dataset, include_multiturn_conversations)
...
def format_dataset(dataset_partition, include_multi_turn_conversations):
    ...
    for entry in dataset_partition:
        ...
        document = f"Document: {entry['article']}\n"

        formatted_ds.append(
            {
                "instruction": prompt + document + "Summary:",
                "input": "",
                "output": entry["highlights"]
            }
        )
